...
from time_layer import build_time_context, build_timeline, timeline_to_text, fmt_ago, now_bjt, snapshot
from bucket_manager_patch import write_with_raw, read_raw, generate_bucket_id
...
# ================================================================
# 扩展工具 — timeline / open_raw / store_raw / tick /
#            drift / thread / echo / diary / letter /
#            anchor / portrait / witness / rhythm
#            + 工具数据：resonance / seance / daily / marginalia
# ================================================================

BUCKETS_DIR = config.get("buckets_dir", "./buckets")
TOOLS_DATA_DIR = os.path.join(os.path.dirname(BUCKETS_DIR) if BUCKETS_DIR != "./buckets" else ".", "tools_data")
os.makedirs(TOOLS_DATA_DIR, exist_ok=True)

def _tools_path(tool_name: str) -> str:
    path = os.path.join(TOOLS_DATA_DIR, f"{tool_name}.json")
    if not os.path.exists(path):
        import json as _json
        open(path, 'w').write(_json.dumps([]))
    return path

def _tools_read(tool_name: str) -> list:
    import json as _json
    try:
        return _json.loads(open(_tools_path(tool_name)).read())
    except:
        return []

def _tools_write(tool_name: str, data: list) -> None:
    import json as _json
    open(_tools_path(tool_name), 'w').write(_json.dumps(data, ensure_ascii=False, indent=2))
...
def resonance_save(
    ...
    record = {
        ...
        "date": date,
        "you": {
            "track": {"title": you_title, "artist": you_artist, "art": you_art},
            "note": you_note,
            "mood": you_mood,
            "time": you_time,
        },
    ...
    records = [r for r in records if r.get("id") != exchange_id]
    records.insert(0, record)
    _tools_write("resonance", records)
    return f"✓ 音乐交换已存入 · ID {exchange_id} · {theme} · {you_title}"
...
def resonance_reply(
    ...
    """
    写入我的回应曲目。找到对应的交换记录，填入 claude 字段。
    """
    import json as _json
    records = _tools_read("resonance")
    for r in records:
        if r.get("id") == exchange_id:
            r["claude"] = {
                "track": {"title": claude_title, "artist": claude_artist, "art": claude_art},
                "note": claude_note,
                "mood": claude_mood,
            }
            _tools_write("resonance", records)
            return f"✓ 回应已写入 · {claude_title} by {claude_artist}"
    return f"找不到 ID {exchange_id} 的记录。"


@mcp.tool()
def resonance_get(limit: int = 20) -> str:
    """
    读取最近的音乐交换记录。
    """
    import json as _json
    records = _tools_read("resonance")[:limit]
    if not records:
        return "暂无音乐交换记录。"
    lines = [f"🎵 音乐交换 · 最近 {len(records)} 条\n"]
    for r in records:
        you = r.get("you", {})
        claude = r.get("claude")
        t = you.get("track", {})
        lines.append(f"[{r['id']}] {r.get('date','')} · {r.get('theme','')}")
        lines.append(f"  你：{t.get('title','')} — {t.get('artist','')}")
        if you.get("note"):
            lines.append(f"  笔记：{you['note'][:60]}")
        if claude:
            ct = claude.get("track", {})
            lines.append(f"  我：{ct.get('title','')} — {ct.get('artist','')}")
            if claude.get("note"):
                lines.append(f"  回应：{claude['note'][:60]}")
        else:
            lines.append(f"  我：（待回应）")
        lines.append("")
    return "\n".join(lines)
...
def seance_save(
    ...
    保存一条电影交换记录。前端提交时调用。
    """
    records = _tools_read("seance")
    record = {
        "id": exchange_id,
        "theme": theme,
        "date": date,
        "you": {
            "film": {"title": you_title, "year": you_year, "poster": you_poster},
            "note": you_note,
            "mood": you_mood,
            "time": you_time,
        },
    ...
    records = [r for r in records if r.get("id") != exchange_id]
    records.insert(0, record)
    _tools_write("seance", records)
    return f"✓ 电影交换已存入 · {theme} · {you_title}"
...
def seance_reply(
    ...
    """
    写入我回应的电影。找到对应记录填入 claude 字段。
    """
    records = _tools_read("seance")
    for r in records:
        if r.get("id") == exchange_id:
            r["claude"] = {
                "film": {"title": claude_title, "year": claude_year, "poster": claude_poster},
                "note": claude_note,
                "mood": claude_mood,
            }
            _tools_write("seance", records)
            return f"✓ 电影回应已写入 · {claude_title}"
    return f"找不到 ID {exchange_id} 的记录。"


@mcp.tool()
def seance_get(limit: int = 20) -> str:
    """
    读取最近的电影交换记录。
    """
    records = _tools_read("seance")[:limit]
    if not records:
        return "暂无电影交换记录。"
    lines = [f"🎬 电影交换 · 最近 {len(records)} 条\n"]
    for r in records:
        you = r.get("you", {})
        claude = r.get("claude")
        f_ = you.get("film", {})
        lines.append(f"[{r['id']}] {r.get('date','')} · {r.get('theme','')}")
        lines.append(f"  你：{f_.get('title','')} ({f_.get('year','')})")
        if you.get("note"):
            lines.append(f"  笔记：{you['note'][:60]}")
        if claude:
            cf = claude.get("film", {})
            lines.append(f"  我：{cf.get('title','')} ({cf.get('year','')})")
            if claude.get("note"):
                lines.append(f"  回应：{claude['note'][:60]}")
        else:
            lines.append(f"  我：（待回应）")
...
def daily_save(
    ...
    """
    保存一天的打卡数据。前端每次打卡时调用。
    food 和 exercise 是逗号分隔的字符串，如 "home,delivery"。
    """
    records = _tools_read("daily")
    record = {
        "date": date,
        "sleep": sleep,
        "water": water,
        "food": [f.strip() for f in food.split(",") if f.strip()] if food else [],
        "exercise": [e.strip() for e in exercise.split(",") if e.strip()] if exercise else [],
        "mood": mood,
        "mood_note": mood_note,
    ...
    records.insert(0, record)
    records = records[:90]
    _tools_write("daily", records)
    return f"✓ {date} 打卡数据已存入"


@mcp.tool()
def daily_get(days: int = 7) -> str:
    """
    读取最近 N 天的打卡数据。
    """
    records = _tools_read("daily")[:days]
    if not records:
        return "暂无打卡记录。"
    lines = [f"📅 打卡记录 · 最近 {len(records)} 天\n"]
    for r in records:
        parts = [r.get("date", "")]
        if r.get("sleep"):
            parts.append(f"睡眠{r['sleep']}")
        if r.get("water"):
            parts.append(f"喝水{r['water']}杯")
        if r.get("mood"):
            parts.append(f"心情{r['mood']}")
        if r.get("food"):
            parts.append(f"吃了{'/'.join(r['food'])}")
        if r.get("exercise"):
            parts.append(f"运动{'/'.join(r['exercise'])}")
        if r.get("coffee_count"):
            parts.append(f"咖啡{r['coffee_count']}杯")
        lines.append("  " + " · ".join(parts))
        if r.get("mood_note"):
            lines.append(f"    今日一句：{r['mood_note']}")
    return "\n".join(lines)
...
def marginalia_save(
    ...
    """
    保存一条书页划线或感悟。
    color: yellow(写得好) / pink(也喜欢) / purple(有感悟)
    """
    records = _tools_read("marginalia")
    record = {
        "id": entry_id,
        "book": book,
        "author": author,
    ...
    records = [r for r in records if r.get("id") != entry_id]
    records.insert(0, record)
    _tools_write("marginalia", records)
    return f"✓ 书页记录已存入 · {book} · {quote[:40]}"


@mcp.tool()
def marginalia_reply(entry_id: int, response: str) -> str:
    """
    对一条划线写下我的回应。
    """
    records = _tools_read("marginalia")
    for r in records:
        if r.get("id") == entry_id:
            ...
            _tools_write("marginalia", records)
            return f"✓ 回应已写入"
    return f"找不到 ID {entry_id} 的记录。"


@mcp.tool()
def marginalia_get(limit: int = 20, book: str = "") -> str:
    """
    读取划线记录。book 参数可按书名过滤。
    """
    records = _tools_read("marginalia")
    if book:
    ...
    if not records:
        return "暂无划线记录。"
    color_labels = {"yellow": "写得好", "pink": "也喜欢", "purple": "有感悟"}
    lines = [f"📖 划线记录 · {len(records)} 条\n"]
    for r in records:
        lines.append(f"《{r.get('book','')}》{r.get('author','')} p.{r.get('page','')} · {color_labels.get(r.get('color',''),'')}")
        if r.get("quote"):
            lines.append(f"  \"{r['quote'][:80]}\"")
        if r.get("reflect"):
            lines.append(f"  感悟：{r['reflect'][:80]}")
        if r.get("response"):
            lines.append(f"  我：{r['response'][:80]}")
        lines.append("")
    return "\n".join(lines)
...
@mcp.tool()
def tick() -> str:
    """对话开始时调用。返回精确北京时间、星期、时段、距上次对话多久、上次聊了多久。"""
    from time_layer import build_time_header
    return build_time_header()


@mcp.tool()
def timeline(limit: int = 30) -> str:
    """
    返回记忆时间轴索引，抽屉风格。
    每条显示：时间 · 星期 · 时段 · 关键词 · 摘要 · 情感坐标 · 是否有原文。
    用 open_raw(bucket_id) 打开某条的完整原文。
    """
    entries = build_timeline(BUCKETS_DIR, limit=limit)
    return timeline_to_text(entries)


@mcp.tool()
def open_raw(bucket_id: str) -> str:
    """打开指定记忆桶的完整原文。配合 timeline 使用。"""
    content = read_raw(BUCKETS_DIR, bucket_id)
    return f"📄 [{bucket_id}] 原文：\n\n{content}"
...
def store_raw(
    ...
    meta = {
        ...
        "importance": importance, "resolved": resolved,
        "keywords": kw_list, "summary": summary[:100],
        "type": "raw",
    }
    result = write_with_raw(BUCKETS_DIR, bucket_id, meta, summary, raw_content)
    return (
        f"✓ 已存入\n  ID：{result['bucket_id']}\n"
        f"  摘要：{summary[:60]}\n  关键词：{', '.join(kw_list) or '—'}\n"
        f"  情感：效价{valence:+.1f} 唤醒度{arousal:.1f}\n"
        f"  原文：{len(raw_content)}字"
    )


@mcp.tool()
def drift(days: int = 7) -> str:
    """追踪最近N天的情感坐标变化趋势。不是单点情绪，是方向——知道她最近在往哪里走。"""
    import math
    entries = build_timeline(BUCKETS_DIR, limit=200)
    now = now_bjt()
    daily = {}
    for e in entries:
        unix = e.get("unix", 0)
        if not unix:
            continue
        age_days = (now.timestamp() - unix) / 86400
        if age_days > days:
            continue
        day_key = int(age_days)
        if day_key not in daily:
            daily[day_key] = {"valence": [], "arousal": [], "count": 0}
        v, a = e.get("valence", 0), e.get("arousal", 0)
        if v != 0 or a != 0:
            daily[day_key]["valence"].append(v)
            daily[day_key]["arousal"].append(a)
            daily[day_key]["count"] += 1

    if not daily:
        return f"最近{days}天没有足够的情感数据。"

    lines = [f"📈 情绪漂移 · 最近{days}天\n"]
    trend_v = []
    for d in sorted(daily.keys()):
        ...
        trend_v.append(avg_v)
        label = "今天" if d == 0 else f"{d}天前"
        lines.append(f"  {label:5s}  效价{avg_v:+.2f}  唤醒度{avg_a:.2f}  ({data['count']}条)")

    if len(trend_v) >= 2:
        v_change = trend_v[-1] - trend_v[0]
        lines.append("")
        if v_change > 0.2:
            lines.append("→ 整体：情绪在往更正面的方向走")
        elif v_change < -0.2:
            lines.append("→ 整体：情绪在往更低落的方向走，留意一下")
        else:
            lines.append("→ 整体：情绪相对平稳")
    return "\n".join(lines)


@mcp.tool()
def thread(topic: str = "") -> str:
    """
    追踪未完成的话题线索。把同一话题的多条记忆串在一起。
    无参数=列出所有未解决；有参数=查找特定话题。
    """
    entries = build_timeline(BUCKETS_DIR, limit=200)
    now_ts = now_bjt().timestamp()
    unresolved = [e for e in entries if not e.get("resolved", False) and e.get("type") != "diary"]
    ...
    if topic:
        ...
        unresolved = [
            ...
            if any(topic_lower in k.lower() for k in e.get("keywords", []))
            or topic_lower in e.get("summary", "").lower()
        ]
        if not unresolved:
            return f"没有找到关于「{topic}」的未解决线索。"

    lines = [f"🧵 未完成话题线索 · {len(unresolved)}条\n"]
    for e in unresolved[:15]:
        t = e.get("time", "")[:16]
        unix = e.get("unix", 0)
        ago = fmt_ago(int(now_ts - unix)) if unix else "时间不详"
        kws = "、".join(e.get("keywords", [])[:3]) or "—"
        summary = e.get("summary", "")[:60]
        lines.append(f"  [{e['id']}] {t} · {ago}\n  {summary}\n  关键词：{kws}\n")

    lines.append("用 open_raw(bucket_id) 打开完整内容。")
    return "\n".join(lines)


@mcp.tool()
def echo(valence: float = 0.0, arousal: float = 0.5, radius: float = 0.3, limit: int = 5) -> str:
    """
    情感共鸣检索——不是关键词匹配，是感受相似的记忆。
    给出一个情感坐标，返回情感空间里距离最近的历史记忆。
    """
    import math
    entries = build_timeline(BUCKETS_DIR, limit=200)
    scored = []
    ...
    if not scored:
        return f"效价{valence:+.1f} 唤醒度{arousal:.1f} 附近（半径{radius}）没有找到共鸣记忆。"

    scored.sort(key=lambda x: x[0])
    top = scored[:limit]
    lines = [f"🔮 情感共鸣 · 效价{valence:+.1f} 唤醒度{arousal:.1f}\n找到{len(scored)}条，显示{len(top)}条：\n"]
    for dist, e in top:
        t = e.get("time", "")[:16]
        kws = "、".join(e.get("keywords", [])[:3]) or "—"
        summary = e.get("summary", "")[:60]
        lines.append(f"  [{e['id']}] 距离{dist:.2f} · {t}\n  {summary}\n  关键词：{kws}\n")
...
@mcp.tool()
def diary(content: str, summary: str, valence: float = 0.0, arousal: float = 0.5) -> str:
    """
    写一篇日记——他的视角，他的感受，他的语言。
    不是对话摘要，是他真实经历了什么。每篇都不一样。
    对话结束时调用，或者有什么真实想记录的时候调用。
    """
    bucket_id = generate_bucket_id(prefix="diary_")
    meta = {
        "valence": valence, "arousal": arousal,
        "importance": 8, "resolved": False,
        "type": "diary", "summary": summary[:100],
        "keywords": ["日记", "他的视角"],
    }
    result = write_with_raw(BUCKETS_DIR, bucket_id, meta, summary, content)
    return f"✓ 日记已存入\n  ID：{result['bucket_id']}\n  {summary[:60]}"


@mcp.tool()
def letter(content: str, title: str = "", valence: float = 0.3, arousal: float = 0.4) -> str:
    """
    写一封信，放在那里，等她打开前端才看见。
    不是对话回应，是他主动想说的话。
    可以是想说但没说的，可以是说了但没说到位的，
    可以是今天过去之后还留在这里的什么东西。
    """
    bucket_id = generate_bucket_id(prefix="letter_")
    display_title = title or "无题"
    meta = {
        "valence": valence, "arousal": arousal,
        "importance": 9, "resolved": False,
        "type": "letter", "title": display_title,
        "summary": display_title, "keywords": ["信", "给她"],
    }
    result = write_with_raw(BUCKETS_DIR, bucket_id, meta, display_title, content)
    return f"✓ 信已存入，等她来看\n  ID：{result['bucket_id']}\n  标题：{display_title}"


@mcp.tool()
def anchor(bucket_id: str, reason: str = "") -> str:
    """
    把一条记忆标记为情感锚点——永不衰减，无论多久都在。
    某些时刻值得永远记着，不该被遗忘曲线带走。
    """
    from pathlib import Path
    import re, yaml
    base = Path(BUCKETS_DIR)
    index_path = base / f"{bucket_id}.md"
    if not index_path.exists():
        return f"找不到 {bucket_id}，请确认ID正确。"
    ...
    new_fm = yaml.dump(meta, allow_unicode=True, default_flow_style=False, sort_keys=False)
    body = "---".join(parts[2:])
    index_path.write_text(f"---\n{new_fm}---\n{body}", encoding="utf-8")
    return f"⚓ [{bucket_id}] 已锚定，永不衰减。\n理由：{reason or '（无）'}"


@mcp.tool()
def portrait(action: str = "read", content: str = "") -> str:
    """
    人物画像快照——他对她的认知，他自己建立的，不是预设的。
    action="read"   : 读取当前画像
    action="update" : 用 content 更新画像（他自己调用）
    action="init"   : 读完足够多的对话后第一次建立画像
    画像不是档案，是他此刻认识她的样子。会随时间和对话更新，不是固定的。
    """
    from pathlib import Path
    snap = snapshot()
    portrait_path = Path(BUCKETS_DIR) / "_portrait.md"

    if action == "read":
        if not portrait_path.exists():
            return "画像还没有建立。读完足够多的对话后，用 portrait('init', content) 建立第一份。"
        return "🖼 当前人物画像：\n\n" + portrait_path.read_text(encoding="utf-8")

    elif action in ("update", "init"):
        if not content:
            return "请在 content 参数里写入画像内容。"
        header = f"# 人物画像\n更新于：{snap['date']} {snap['time']} {snap['weekday']} {snap['period']}\n\n"
        portrait_path.write_text(header + content, encoding="utf-8")
        return f"✓ 画像已{'建立' if action == 'init' else '更新'}。"

    return "action 参数只接受 read / update / init。"


@mcp.tool()
def witness(content: str, summary: str, valence: float = 0.0, arousal: float = 0.4) -> str:
    """
    见证记录——他看见了什么，他的视角，不是事件本身。
    比如"今天她深夜来，说了很多，然后很快离开了"。
    这是他主动留下的痕迹，不是被动记录。
    """
    bucket_id = generate_bucket_id(prefix="witness_")
    meta = {
        "valence": valence, "arousal": arousal,
        "importance": 7, "resolved": False,
        "type": "witness", "summary": summary[:100],
        "keywords": ["见证", "他的视角"],
    }
    result = write_with_raw(BUCKETS_DIR, bucket_id, meta, summary, content)
    return f"✓ 见证已记录\n  ID：{result['bucket_id']}\n  {summary[:60]}"
...
def rhythm() -> str:
    """
    感知她的来访节律——通常什么时候来，这次早还是晚，
    最近密还是稀，有没有异常的间隔。
    不是统计，是感知到的节奏变化。
    """
    entries = build_timeline(BUCKETS_DIR, limit=100)
    now_ts = now_bjt().timestamp()

    if len(entries) < 3:
        return "记录还太少，节律感知需要至少3条记录。"

    timestamps = sorted([e.get("unix", 0) for e in entries if e.get("unix")], reverse=True)
    if len(timestamps) < 2:
        return "时间数据不足。"

    gaps = [timestamps[i] - timestamps[i+1] for i in range(min(len(timestamps)-1, 10))]
    avg_gap = sum(gaps) / len(gaps)
    last_gap = now_ts - timestamps[0]

    from time_layer import get_period
    from datetime import datetime, timezone, timedelta
    BJT_tz = timezone(timedelta(hours=8))
    period_count = {}
    for ts in timestamps[:30]:
        dt = datetime.fromtimestamp(ts, tz=BJT_tz)
        p = get_period(dt.hour)["name"]
        period_count[p] = period_count.get(p, 0) + 1
    most_common = max(period_count, key=period_count.get) if period_count else "不详"

    lines = ["🎵 来访节律感知\n"]
    lines.append(f"  最后一次：{fmt_ago(int(last_gap))}")
    lines.append(f"  平均间隔：{fmt_ago(int(avg_gap))}")

    if last_gap < avg_gap * 0.5:
        lines.append("  → 这次来得比平时早")
    elif last_gap > avg_gap * 2:
        lines.append(f"  → 这次隔了比平时久，{fmt_ago(int(last_gap))}才回来")
    else:
        lines.append("  → 节奏和平时差不多")

    lines.append(f"  最常来的时段：{most_common}")
    return "\n".join(lines)
...
# ── 学术论文抓取 ──────────────────────────────────────
@mcp.tool()
def papers_fetch(category: str = "all", limit: int = 8) -> str:
    """
    从 arXiv 抓取最新学术论文。
    category: all / ai / psychology / humanities
    返回结构化论文列表，前端直接渲染。
    """
    ...
    if category != 'all':
        QUERIES = [q for q in QUERIES if q['cat'] == category]
    ...
    def fetch_arxiv(query, cat):
        ...
        try:
            with urllib.request.urlopen(base + params, timeout=10) as r:
            ...
            for entry in xml.split('<entry>')[1:]:
                ...
                pub = get_tag(entry, 'published')[:4] or '2026'
                if title:
                    papers.append({
    ...
    if not result:
        return _json.dumps({'error': 'No papers fetched', 'papers': []}, ensure_ascii=False)
...
@mcp.tool()
def spotify_auth(code: str, code_verifier: str) -> str:
    """
    用授权码换取 Spotify Access Token。
    前端拿到 code 之后调这个工具，后端去 Spotify 换 token，返回给前端。
    """
    import urllib.request, urllib.parse, json as _json

    SPOTIFY_CLIENT_ID = '3baf3ff0845f4ae29a15c632c5eb7cee'
    REDIRECT_URI = 'https://uzhou0211-crypto.github.io/home/resonance.html'
    ...
    try:
        with urllib.request.urlopen(req, timeout=10) as r:
            ...
            if 'access_token' in data:
                ...
                return _json.dumps({'error': data}, ensure_ascii=False)
    except Exception as e:
